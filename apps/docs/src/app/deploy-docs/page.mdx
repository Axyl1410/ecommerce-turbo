...
3. **Build command**: `cd ../.. && pnpm build:docs` (if root is `apps/docs`)
   - Or: `pnpm build` (if root is monorepo root)
...
2. **Build command**: `cd ../.. && pnpm build:docs`
...
**Solution:** Use `pnpm build:docs` or `turbo build --filter=docs` instead of `turbo build`.

### Postinstall script fails

**Solution:** Ensure `SKIP_PRISMA_GENERATE=1` is set before running `pnpm install`.
