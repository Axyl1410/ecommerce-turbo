...
const createCart = (overrides?: Partial<{ userId: string | null; sessionId: string | null }>) =>
  Cart.create({
    id: "cart-1",
    userId: "user-1",
    sessionId: null,
...
describe("Cart Entity", () => {
  ...
  it("merges quantities when adding duplicate variants", () => {
    ...
    const duplicate = buildCartItem({ variantId: "variant-dup", quantity: 3, priceAtAdd: 2999 });
