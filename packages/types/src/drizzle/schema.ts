...
import { boolean, decimal, doublePrecision, foreignKey, integer, jsonb, pgEnum, pgTable, primaryKey, text, timestamp, uniqueIndex } from 'drizzle-orm/pg-core'

export const ProductStatus = pgEnum('ProductStatus', ['DRAFT', 'PUBLISHED', 'ARCHIVED'])

export const PaymentStatus = pgEnum('PaymentStatus', ['PENDING', 'PAID', 'FAILED', 'REFUNDED', 'PARTIALLY_REFUNDED'])

export const OrderStatus = pgEnum('OrderStatus', ['PENDING', 'CONFIRMED', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'RETURNED', 'REFUNDED'])

export const ReviewStatus = pgEnum('ReviewStatus', ['PENDING', 'APPROVED', 'REJECTED'])

export const CouponType = pgEnum('CouponType', ['PERCENT', 'FIXED', 'FREE_SHIPPING'])
...
export const Order = pgTable('order', {
	...
	discountAmount: decimal('discountAmount', { precision: 65, scale: 30 }).notNull(),
...
export const CouponApplicableProduct = pgTable('coupon_applicable_product', {
	...
	'CouponApplicableProduct_cpk': primaryKey({
		...
		columns: [CouponApplicableProduct.couponId, CouponApplicableProduct.productId]
	})
}));

export const CouponApplicableCategory = pgTable('coupon_applicable_category', {
	couponId: text('couponId').notNull(),
	...
	'CouponApplicableCategory_cpk': primaryKey({
		...
		columns: [CouponApplicableCategory.couponId, CouponApplicableCategory.categoryId]
	})
}));

export const CouponExcludedProduct = pgTable('coupon_excluded_product', {
	couponId: text('couponId').notNull(),
	productId: text('productId').notNull()
	...
	'CouponExcludedProduct_cpk': primaryKey({
		...
		columns: [CouponExcludedProduct.couponId, CouponExcludedProduct.productId]
	})
}));

export const CouponExcludedCategory = pgTable('coupon_excluded_category', {
	couponId: text('couponId').notNull(),
	categoryId: text('categoryId').notNull()
	...
	'CouponExcludedCategory_cpk': primaryKey({
		...
		columns: [CouponExcludedCategory.couponId, CouponExcludedCategory.categoryId]
	})
}));
...
export const WishlistItem = pgTable('wishlist_item', {
	...
	'wishlist_item_product_fkey': foreignKey({
		...
		.onDelete('cascade')
		.onUpdate('cascade'),
	'WishlistItem_userId_productId_unique_idx': uniqueIndex('WishlistItem_userId_productId_key')
...
export const CouponApplicableProductRelations = relations(CouponApplicableProduct, ({ one }) => ({
	coupon: one(Coupon, {
		relationName: 'CouponToCouponApplicableProduct',
...
export const CouponApplicableCategoryRelations = relations(CouponApplicableCategory, ({ one }) => ({
	coupon: one(Coupon, {
		relationName: 'CouponToCouponApplicableCategory',
...
export const CouponExcludedProductRelations = relations(CouponExcludedProduct, ({ one }) => ({
	coupon: one(Coupon, {
		relationName: 'CouponToCouponExcludedProduct',
...
export const CouponExcludedCategoryRelations = relations(CouponExcludedCategory, ({ one }) => ({
	coupon: one(Coupon, {
		relationName: 'CouponToCouponExcludedCategory',
